...
interaction.maker <- function(df,factor1=NULL,factor2=NULL,value, credvalue = 0.90){
        ...
        if(is.null(factor1) == FALSE & is.null(factor2) == FALSE){
                ...
                #First factor
                results <- as.data.frame(matrix(rep(NA, times = length(return.results@f.levels1)*5),
                                                nrow = length(return.results@f.levels1),ncol = 5))
                ...
                #Seccond factor
                results <- as.data.frame(matrix(rep(NA, times = length(return.results@f.levels2)*5),
                                                nrow = length(return.results@f.levels2),ncol = 5))
                ...
                #Interaction
                results <- as.data.frame(matrix(rep(NA, times = length(return.results@f.levels.int)*5),
                                                nrow = length(return.results@f.levels.int),ncol = 5))
...
post_hoc_plot <- function(post.hoc.object,
        ...
        if(post.hoc.object@f.levels1.null == FALSE){
                ...
                # y_axis_max <- (RobustMax(w.data.f1.vals[,"Mean"]) + RobustMax(w.data.f1.vals[,"HDIhigh"]))
                # y_axis_min <- (RobustMin(w.data.f1.vals[,"Mean"]) - RobustMin(w.data.f1.vals[,"HDIlow"]))
                y_axis_max <- RobustMax(w.data.f1.vals[,"HDIhigh"])+2
                y_axis_min <- RobustMin(w.data.f1.vals[,"HDIlow"])-2
        ...
        if(post.hoc.object@f.levels2.null == FALSE){
                ...
                #y_axis_max <- (RobustMax(w.data.f2.vals[,"Mean"]) + RobustMax(w.data.f2.vals[,"HDIhigh"]))
                #y_axis_min <- (RobustMin(w.data.f2.vals[,"Mean"]) - RobustMin(w.data.f2.vals[,"HDIlow"]))
                y_axis_max <- RobustMax(w.data.f2.vals[,"HDIhigh"])+2
                y_axis_min <- RobustMin(w.data.f2.vals[,"HDIlow"])-2
        ...
        if(post.hoc.object@f.levels1.null == FALSE & post.hoc.object@f.levels2.null == FALSE){
                ...
                y_axis_max <- (RobustMax(w.data.inter.vals[,"Mean"]) + RobustMax(w.data.inter.vals[,"HDIlow"]))
                y_axis_min <- (RobustMin(w.data.inter.vals[,"Mean"]) - RobustMin(w.data.inter.vals[,"HDIlow"]))
                #y_axis_max <- max(w.data.inter.vals[,"HDIhigh"])+2
                #y_axis_min <- min(w.data.inter.vals[,"HDIlow"])-2
