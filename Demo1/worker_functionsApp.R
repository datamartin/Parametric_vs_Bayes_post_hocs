...
#setwd("C:/Users/Martin/ownCloud/Projects/R_APP16/app/Parametric_vs_Bayes_post_hocs")
#Helper functions
...
post_hoc <- function(interaction.object,
  ...
  if(interaction.object@f.levels1.null == FALSE & interaction.object@f.levels2.null == FALSE){
    ...
    for(i in c(1:dim(comp.f1)[1])){
      ...
      BF.res = BayesFactor::ttestBF(formula = as.formula(comp.formula), data = subset)
...
#TESTING
# a <- interaction.maker(df = TEST_FILE,factor1 = "F1", factor2 = "F2", value = "VAL")
...
#Playground
# library(BEST) #bayes joonistel tooks hdi välja. aga see on suhteliselt lai. kas on mõtet?
# #hdi piir bayesil 68% 
# 
